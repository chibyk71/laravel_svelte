<script lang="ts">
    ...
    const handleSubmit = async (data: any) => {
        try {
            ...
            const responseData = response.data // For JSON responses

            if (responseData.status) {
                dispatcher("success",{msg:responseData.message})
            }else {        
                dispatcher("failed",{msg:responseData.message})
            }
            return responseData;
        } catch (error) {
            dispatcher("error",{msg:error})
            console.error(`HTTP error! status: ${error}`);
        }

        let x = await clearFiles()
        dispatcher("done")
    };
